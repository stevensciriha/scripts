...
get_configs(){
   ...
   set -A Configs_List "$BPC_HOME/slf-config.xml" "/etc/corosync/corosync.conf" "/opt/bpc/is/DRASLF/config/dra-slf-config.xml" "/etc/logrotate-traffix.conf" "/etc/rsyslog.conf" "/etc/snmp/snmpd.conf" "/etc/ssh/sshd_config" "/etc/sysconfig/ip6tables" "/etc/sysconfig/iptables" "/opt/traffix/config/fep*.xml" "/opt/traffix/config/defaultLBConfiguration.xml" "/opt/traffix/config/*" "/etc/keepalived/keepalived.conf" "/etc/ipsec*" "/opt/traffix/sdc/data/backup/*"
...
ACTIVE_PARTITION=`/opt/bpc/is/$SLF_INSTANCE_NAME/bin/rgmgr list|grep " ACTIVE " |awk '{sub(/^/, " ")};1' | sed 's/  */ /'g |cut -d' ' -f2 |tail -1`
