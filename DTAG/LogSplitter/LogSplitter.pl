...
# v1.1 ssciriha Mar 3 2012
#     - line 63: added exit command to invalid interface log loop
#     - line 146: Moved Pid file creation to line 146 to ensure pid file isnt created until script succesfully starts up

# Read in the 2 command line arguments provided with the script
$inputFilename = shift(@ARGV);
$configFilename = shift(@ARGV);

# Create signal handler for all the termination signals
use sigtrap 'handler' => \&cleanAndExit, 'INT', 'ABRT', 'QUIT', 'TERM', 'KILL';
...
$date = `date`;
chomp($date);
print LOG "$date: Starting up logsplitting script ...\n\n";
...
#this subroutine will be executed each time the script is terminated and ensure all the filehandles are closed cleanly
sub cleanAndExit(){

         $date = `date`;
         chomp($date);
         print LOG "$date: LogSplitter has been stopped...\n\n";
         foreach (@interfacelogfile)
         {
         $absolute_logfile = "$logfile_dir$_";
         close($_);
