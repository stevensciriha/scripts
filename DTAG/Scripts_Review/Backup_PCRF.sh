...
#          echo "DEBUG:STATE:$STATE" >> $LOG
          ...
          then
            ...
            # execute the policyExport for all 3 projects
         ...
         #   do
           #   if [ "$XC_USERNAME" = "" ]
            #  then
             #   echo "Please enter the name and password of an account with policy write privileges to the RMA"
#    
 #               PC_USERNAME=""
  #              while [ ! $PC_USERNAME ]
   #             do
    #              read XC_USERNAME?"Username: "
     #             if [ ! $XPCUSERNAME ];then
      #              echo ""
       #           fi
        #        done
         #       stty -echo
          #      read XPC_PASSWORD?"Password: "
           #     stty echo
    
                export XPC_USERNAME
                export XPC_PASSWORD
        #      fi
#              echo "DEBUG: Collect uname and pw: $XP_USERNAME:$XPC_PASSWORD" >> $LOG
          #    echo
         #     break
         # done
...
#            echo "DEBUG:Instance $arg not online" >> $LOG
          fi
        fi
      fi
    done
    # if we get here, it is because we could not find an online instance to export rule for.
    echo "WARNING: There are no online instnaces found. No RMA rules will be exported." >> $LOG
  else
