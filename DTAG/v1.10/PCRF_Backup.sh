...
BACKUP_DIR="/doxstore1/backup"

#set temp directory under /backup
TEMP_DIR="$BACKUP_DIR/temp"

#set file list config location
FILE_LIST="/opt/Amdocs/backup_scripts/filelist.txt"
...
#  fi
 # generate backup file name
  BACKUP_FILE="$BACKUP_DIR/BACKUP_`hostname`_`date '+PCRF.%d.%m.%y'`"
#  export BACKUP_FILE

# Check if temp directory has been created if delete it and recreate it, if not create it
if [ -d $TEMP_DIR ]
  then
